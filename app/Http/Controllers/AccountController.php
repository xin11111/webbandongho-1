...
class AccountController extends Controller
{
    ...
    public function postLoginCustomer(Request $request){
        ...
        if(Auth::guard('account_customer')->attempt($data,$remember)){
            ...
            return redirect(($urlprev!=url('/login')&&$urlprev!=url('/signup')&&$urlprev!=null)?session('urlprev'):url('/'))->with(['typeMsg'=>'success','msg'=>'Chào mừng '.$account->person->full_name.' trở lại !']);
        }
        else return back()->with(['typeMsg'=>'danger','msg'=>'Tài khoản/mật khẩu không chính xác']);     
    }
    ...
    public function postLoginAdmin(Request $request){
    	...
    	if(Auth::guard('account_admin')->attempt($data,$remember)){
    		...
    		return redirect(url('/admin-page'))->with(['typeMsg'=>'success','msg'=>'Chào mừng '.$account->person->full_name.' trở lại !']);
    	}
    	else return back()->with(['typeMsg'=>'danger','msg'=>'Đăng nhập thất bại']);    	
    }
